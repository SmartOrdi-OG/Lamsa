...
export default async function handler(req, res) {
  ...
  // === EXPERIMENTAL: apply an inspiration-gallery style photo to the user's
  // own room photo, via Nano Banana Pro's multi-image input, skipping Flux
  // and the credit deduction entirely. Test-only path for the inspirations
  // gallery simulation (test-inspirations.html) — validates whether
  // reference-image style adherence is good enough before this becomes a
  // real feature. Remove once that decision is made.
  if (reference_image_url) {
    if (!image_url) return res.status(400).json({ error: 'image_url is required' });

    const nanoBody = buildNanoBody({ prompt, image_url, count: 1 });
    nanoBody.image_urls = [image_url, reference_image_url];
    console.log('[api/generate] experimental style-reference stage — submitting to nano-banana-pro:', JSON.stringify(nanoBody));
    ...
    try {
      ...
      console.error('[api/generate] experimental style-reference submit failed:', err.message);
