...
- [x] Fix bathroom fixtures surviving into non-bathroom redesigns, in two rounds. First attempt named the offending items explicitly ("if a shower, bathtub, sink, toilet... is visible, REMOVE it") — this backfired: the next test generation added a *bathtub* that wasn't there before, because image-edit models are known to respond to salient nouns in a prompt more than to negation, so naming "bathtub" even to forbid it made the model render one. Fixed properly with a positive allow-list instruction instead ("strip the room to its bare shell, then place ONLY the items listed below — nothing else"), which never names bathroom fixtures at all.
