...
- [x] Fix bathroom fixtures surviving into non-bathroom redesigns — three rounds so far, still being tracked. Round 1 named the offending items explicitly ("if a shower, bathtub, sink, toilet... is visible, REMOVE it") — backfired: the next test added a *bathtub* that wasn't there before (image-edit models respond to salient nouns more than negation). Round 2 switched to a positive allow-list instruction ("strip the room to its bare shell, then place ONLY the items listed below") without naming any fixture — user's next test still showed a shower head, now fused onto the bed frame. Round 3: `callGenerate()` in `lamsa-bilingual.html` was still using `guidance_scale: 3.5` (the same low value meant for light touch-ups) despite this flow fully stripping and rebuilding the room's furniture — raised to `7.5` to match the value that fixed the identical symptom in `rearrange.html`'s Full Redesign mode, so the prompt carries more weight over the source pixels. Awaiting user confirmation this round actually resolves it.
