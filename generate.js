export default async function handler(req, res) {
  ...
  try {
    const { prompt, image_url, num_images = 1 } = req.body;
    ...
    // Use Kontext Pro — built for image editing with instruction following
    ...
    if (!response.ok) {
      ...
      console.error('Kontext error:', response.status, errText);
